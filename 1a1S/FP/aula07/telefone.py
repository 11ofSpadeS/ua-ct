# Complete este programa como pedido no guião da aula.

def listContacts(dic):
    """Print the contents of the dictionary as a table, one item per row."""
    print("{:>12s} : {}".format("Numero", "Nome"))
    for num in dic:
        print("{:>12s} : {}".format(num, dic[num]))

def filterPartName(contacts, partName):
    """Returns a new dict with the contacts whose names contain partName."""
    ...

def addContacto(contactos):
    while True:
        num = input("Numero de telemovel?")
        nome = str(input("Nome?"))
        if num in contactos:
            print("O Numero ja se encontra na lista de contactos")
        else:
            contactos.setdefault(num , nome)
            break

def removeContacto(contactos):
    while True:
        num = input("Numero a remover?")
        if num in contactos:
            contactos.pop(num)
            break
        else:
            print("O Numero não se encontra guardado nos contactos")

def numSearch(contactos):
    num = input("Numero?")
    if num in contactos:
        print(contactos.get(num))
    else:
        print("O contacto não se encontra na lista de contactos")
        print(num)


def menu():
    """Shows the menu and gets user option."""
    print()
    print("(L)istar contactos")
    print("(A)dicionar contacto")
    print("(R)emover contacto")
    print("Procurar (N)úmero")
    print("Procurar (P)arte do nome")
    print("(T)erminar")
    op = input("opção? ").upper()   # converts to uppercase...
    return op


def main():
    """This is the main function containing the main loop."""

    # The list of contacts (it's actually a dictionary!):
    contactos = {"234370200": "Universidade de Aveiro",
        "727392822": "Cristiano Aveiro",
        "387719992": "Maria Matos",
        "887555987": "Marta Maia",
        "876111333": "Carlos Martins",
        "433162999": "Ana Bacalhau"
        }

    op = ""
    while op != "T":
        op = menu()
        if op == "T":
            print("Fim")
        elif op == "L":
            print("Contactos:")
            listContacts(contactos)
        elif op == "A":
            addContacto(contactos)
            print("Contacto adicionado com sucesso" )
            print("Contactos:")
            listContacts(contactos)
        elif op == "R":
            removeContacto(contactos)
            print("Contacto removido com sucesso")
            print("Contactos:")
            listContacts(contactos)
        elif op == "N":
            numSearch(contactos)

    

# O programa começa aqui
main()

