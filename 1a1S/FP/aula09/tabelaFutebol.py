...
def main():
    ...
    tab = sorted(tabela, reverse=True, key=lambda x: pontos(x))
    printTabela(tab)

    # c)
    # Acrescente os argumentos adequados à função sorted para
    # obter uma tabela ordenada por ordem decrescente da diferença GM-GS:
    tab = sorted(tabela, reverse=True, key=lambda x: x[GM] - x[GS])
    printTabela(tab)
    ...
    tab = sorted(sorted(tabela, reverse=True, key=lambda x: x[GM] - x[GS]), reverse=True, key=lambda x: pontos(x))
    printTabela(tab)
